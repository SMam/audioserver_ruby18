...
class Audiodata
  ...
  def clip_data(data)
    ...
    bcc = 0
    separator_count = 0
    while data[i] != 2     # <stx>までスキップする
      i += 1
    end
    i += 1
    return "machine_error" if data[i] != 0x37 # 検査機器コードの確認
    bcc ^= data[i]
    i += 1

    return "exam_error" if data[i] != 0x40 # 検査項目コードの確認
    bcc ^= data[i]
    i += 1
    ...
    if bcc != data[i] then              # <bcc>とその手前までのデータのXORを比較
  ...
  def decode_mask(mask)     # 条件コード(mask: 配列)からマスキング条件を読み取る
    result = Array.new
    for i in 0..6
      condition_code1 = mask[i][2,1][0]
      condition_code2 = mask[i][3,1][0]
